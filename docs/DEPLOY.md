...
3. **忘记密码功能**：再跑 `supabase/migrations/002_password_reset.sql`（建 `password_reset_tokens` 表）
4. 确认 **Table Editor** 出现 5 张表：`app_users`、`usage_limits`、`generations`、`payment_events`、`password_reset_tokens`
...
配齐 SMTP 后，登录页会出现 **Forgot password?**，流程：发邮件 → 点链接 → `/reset-password` 设新密码。

1. Supabase **SQL Editor** 跑 `supabase/migrations/002_password_reset.sql`（若 001 已跑过，只补这一份即可）
2. 163 邮箱 → **设置 → POP3/SMTP/IMAP** → 开启 **SMTP 服务** → 生成 **授权码**（不是登录密码）
3. 环境变量：
...
SMTP_PASS=你的SMTP授权码
SMTP_FROM=HirePen <你的邮箱@163.com>
```

4. 重启 dev / Vercel Redeploy → `/login` 应出现 **Forgot password?**
5. 测试：http://localhost:3000/forgot-password → 填注册邮箱 → 收信 → 点链接改密码

> 不配 SMTP：忘记密码入口**不显示**，其它登录功能不受影响。
...
CREEM_API_KEY=creem_test_...              # 测试 key；正式用 creem_ 开头
# CREEM_API_BASE_URL=                      # 可选；不填则按 key 前缀自动选 API 地址
...
2. Products → 建 **Pro（订阅 $1.99/月）** 和 **Lifetime（一次性 $29.90）** → Copy 各 product ID
3. Developers → API Keys → 复制 key（测试模式：`creem_test_` 开头）
4. Developers → Webhooks → URL：`https://hirepen.net/api/webhooks/creem`（本地测结账可先不配）

### 9.2 API 地址（测试 vs 正式）

Creem **测试**与**正式**环境 API 不同，不能混用：

| 环境 | API Key 前缀 | API 地址（`CREEM_API_BASE_URL`） |
|------|--------------|----------------------------------|
| 测试 | `creem_test_` | `https://test-api.creem.io` |
| 正式 | `creem_`（非 test） | `https://api.creem.io` |

**默认行为**：不填 `CREEM_API_BASE_URL` 时，代码根据 key 前缀**自动选择**上表地址。  
只有特殊需求时才手动覆盖，例如：
...
参考：[Creem Test Mode](https://docs.creem.io/getting-started/test-mode) · [API 介绍](https://docs.creem.io/api-reference/introduction)

### 9.3 环境变量

```
CREEM_API_KEY=creem_test_...
CREEM_WEBHOOK_SECRET=whsec_...           # 上线后再填亦可
NEXT_PUBLIC_CREEM_PRODUCT_PRO=prod_xxx   # $1.99/月
NEXT_PUBLIC_CREEM_PRODUCT_LIFETIME=prod_yyy  # $29.90
```

配齐后 `/pricing` 按钮变为真实结账。本地测试保持 **测试模式开启**，付完不会扣真钱。

### 9.4 上线切正式

1. Creem 关闭测试模式 → 创建**正式** API Key + 产品转 live
2. Vercel 换成正式 `CREEM_API_KEY`、正式 product ID
3. （可选）设 `CREEM_API_BASE_URL=https://api.creem.io`，或删掉该变量让代码根据正式 key 自动用 `api.creem.io`
4. Redeploy
...
### 部署前
- [ ] Supabase 已跑 `001_initial.sql` + `002_password_reset.sql`（要用忘记密码时）
- [ ] 本地 `npm run dev`：Generate + 注册/登录均正常
- [ ] `AUTH_SESSION_SECRET` 已生成
- [ ] （可选）163 SMTP 已配，忘记密码流程测通
- [ ] （可选）Creem 测试 key + 两个 product ID，`/pricing` 能跳转结账
...
### Q: Creem 报 `Invalid API Key`？
→ 测试 key（`creem_test_`）必须走 `https://test-api.creem.io`。本项目会自动识别；若仍报错，显式设 `CREEM_API_BASE_URL=https://test-api.creem.io` 并确认左下角测试模式开启。

### Q: 忘记密码收不到邮件？
→ 检查 163 **SMTP 授权码**（不是邮箱登录密码）、`SMTP_USER`/`SMTP_PASS`、是否跑过 `002_password_reset.sql`。看垃圾邮件箱。
